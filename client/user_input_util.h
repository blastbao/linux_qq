...
{
/* to_name and data must be end with '\0' */
...
public:
	...
	*@p_input_fields out
		a structure to hold user input fields, the fields contain {to_name, data}
	*@return
		true if user input formation is correct
		false if user input formation is wrong
	*/
	static bool extract(const char* user_input, const int input_len, User_input_field* p_input_fields);
